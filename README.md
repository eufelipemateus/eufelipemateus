# Felipe Mateus | Full Stack Developer 🚀

[<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />](https://www.linkedin.com/in/eufelipemateus/)
[<img src="https://img.shields.io/badge/Portfolio-000?style=for-the-badge&logo=about.me&logoColor=white" />](https://felipemateus.com/eufelipemateus)
[<img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" />](https://wa.me/+552120423551)
[<img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" />](https://www.instagram.com/eufelipemateus)
[<img src="https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white" />](https://www.facebook.com/eufelipemateus)
[<img src="https://img.shields.io/badge/Telegram-26A6E1?style=for-the-badge&logo=telegram&logoColor=white" />](https://t.me/eufelipemateus)

## 👨‍💻 Sobre Mim
Desenvolvedor **Full Stack** apaixonado por backend, baseado no Rio de Janeiro. Com vasta experiência em **Python, Node.js e PHP (Laravel)**, foco na criação de sistemas escaláveis, automações e soluções web de alta performance. 

Atualmente, atuo como desenvolvedor independente e consultor técnico, transformando ideias complexas em código limpo e funcional.

---

## 🛠️ Especialidades e Tecnologias
Para desenvolver soluções escaláveis e com facil manutenção, trabalho com as seguintes stacks:

* **Backend:** Python (Django/Flask), Node.js (Express), PHP (Laravel/Wordpress), Go (Golang).
* **Frontend:** JavaScript (ES6+), HTML5, CSS3, Integração com APIs REST.
* **Bancos de Dados:** MySQL, PostgreSQL, MongoDB, Redis.
* **DevOps & Ferramentas:** Docker, Git/GitHub, WebSockets (Socket.io), Linux.

---

## 🚀 Projetos em Destaque

### 🀄 [Domino Game](https://github.com/eufelipemateus/domino-game)
Desenvolvimento de um motor de jogo de dominó multiplayer em tempo real utilizando **Socket.io** e Node.js. Envolve modelagem de estado do jogo, sincronização em tempo real entre jogadores e gerenciamento de conexões simultâneas via WebSockets.

[Dominó Multiplayer jogo online - Post](https://felipemateus.com/blog/2017/06/domino)

### 📺 [Laravel IPTV CMS](https://github.com/eufelipemateus/laravel-iptv-cms)
Um sistema de gerenciamento de conteúdo (CMS) robusto para serviços de IPTV, construído com o framework **Laravel**. Foco em arquitetura MVC e gestão de dados complexos.

[Canais m3u8 - Post](https://felipemateus.com/blog/2021/08/pacote-gerenciador-de-canais-iptv-no-laravel)

[Canais Costumer - Post](https://felipemateus.com/blog/2021/10/gerenciador-iptv-customer)

### 📥 [TikTok Live Download](https://github.com/eufelipemateus/tiktok-live-download)
Ferramenta de automação escrita em **Go** para captura e download de transmissões ao vivo, explorando concorrência e manipulação de fluxos de vídeo.

---

## 🔍 O que eu costumo desenvolver
- APIs REST e serviços backend de alta performance
- Sistemas em tempo real (WebSockets, streaming, automação)
- Plataformas de conteúdo e mídia
- Integrações entre sistemas e protocolos
- Infraestrutura leve com Docker e Linux


---

## 📩 Vamos conversar?
Estou disponível para novos projetos, parcerias e consultorias técnicas.

* **Website:** [felipemateus.com](https://felipemateus.com)
* **E-mail:** [contato@felipemateus.com](mailto:contato@felipemateus.com)
* **Localização:** Rio de Janeiro, Brasil 🇧🇷
* 📝 **Solicitar Orçamento:** [Clique aqui para descrever seu projeto](https://felipemateus.com/solicite-orcamento)
